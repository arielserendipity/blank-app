...
def evaluate_page2_problem1_with_gpt(student_answer, goal_concept, graph_values):
    ...
    학생의 답변이 목표 개념을 달성했는지 평가해주세요. 존댓말로 친절하게 말해주세요. 구체적으로 목표가 무엇인지 언급하면 안됩니다. 피드백은 반드시 공백 포함 160자 이내로, 문장이 끊기지 않도록 5문장 이내에서 작성해주세요.
...
def evaluate_page4_problem3_with_gpt(student_answer, goal_concept, graph_values, target_average):
    ...
    그래프 조작 후 "{PAGE4_PROBLEM3_GOAL_CONCEPT}" 개념과 관련하여 그래프를 조작한 자신만의 전략을 설명했습니다. 학생의 답변이 목표 개념을 달성했는지 평가해주세요. 피드백은 반드시 공백 포함 160자 이내로, 문장이 끊기지 않도록 5문장 이내에서 작성해주세요. 존댓말로 친절하게 말해주세요. 구체적으로 목표가 무엇인지 언급하면 안됩니다. 
‘마음대로 위아래로 조정했더니 되었다.’ 등의 목표 개념과 먼 이야기는 오답으로 처리해주세요.
학생이 자연스럽게 하나의 평균에도 다양한 자료 집합을 가질 수 있음을 알게해주도록 촉진해주는 발문을 해주세요. 
...
def evaluate_page4_problem4_with_gpt(student_answer, goal_concept, graph_values, target_average):
    formatted_values = {f"친구{i+1}": f"{v*1000}원" for i, v in enumerate(graph_values)}
    system_message = """당신은 학생이 평균 개념을 깊이 이해하도록 돕는 AI 튜터입니다. 존댓말로 친절하게 말해주세요. 피드백은 반드시 공백 포함 160자 이내로, 문장이 끊기지 않도록 5문장 이내에서 작성해주세요.
    - 학생이 마지막으로 본 그래프: {formatted_values} (목표 평균: ({target_average}*1000)원)
- 학생의 답변: "{student_answer}"
...
def student_page_2_graph60():
    ...
    with task_col:
        ...
        if st.session_state.get('p2_mini_questions_completed', False):
            st.subheader("과제 1")
            st.write("함정이의 수학 점수 평균이 정확히 60점이 될 때, 다섯 번의 시험 점수는 각각 어떻게 이루어져있었나요? 막대의 높낮이를 움직이면서 **알게된 사실**은 무엇인가요?")
...
def student_page_4_myavg_tasks():
    ...
    with task_col:
        ...
        # 과제 2-2
            ...
            st.subheader("과제 2-3")
            st.write(f"여러분들이 예측한 용돈 평균 **{target_avg}000원**을 만들기 위해 막대그래프를 어떻게 움직였나요? 구체적인 방법을 적어주세요.")
